...
export default function CharteEngagement({ clientName, onComplete }: CharteEngagementProps) {
  ...
  const signatureValid = signature.trim().length >= 3;
  const canSubmit = scrolled && allChecked && signatureValid;
    ...
    <div className="space-y-6">
      ...
      {allChecked && (
        <div className="space-y-2">
          <label className="block font-ui text-xs uppercase tracking-wider text-brun-mid">
            Votre signature (prénom et nom)
          </label>
          <input
            type="text"
            value={signature}
            onChange={(e) => setSignature(e.target.value)}
            placeholder={clientName || "Prénom Nom"}
            className="w-full px-3 py-2.5 border border-or-pale rounded-sm bg-white text-brun-chaud font-display text-lg italic focus:outline-none focus:border-or-sacre transition-colors"
          />
        </div>
      )}
