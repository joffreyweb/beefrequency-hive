...
import { PhaseDetail, type ClientPhase } from "@/components/admin/ParcoursSection";
...
// Poser des modules à durées libres → régénère la timeline. Chaque phase est ensuite
// cliquable → mêmes onglets que le 103j (Élixirs / Pratiques / Instructions / Check-ins).
export default function CustomTimelineComposer({ clientId }: { clientId: string }) {
  ...
  const loadPhases = useCallback(
    async (syncMods: boolean) => {
      try {
        const r = await fetch(`/api/client-phases?clientId=${clientId}`);
        const d = r.ok ? await r.json() : { phases: [] };
        const ph: ClientPhase[] = d.phases ?? [];
        setPhases(ph);
        if (syncMods && ph.length > 0) {
          setMods(
            ph.map((p) => ({
  ...
  useEffect(() => {
    loadPhases(true).finally(() => setLoading(false));
  }, [loadPhases]);
  ...
  async function regenerate() {
    ...
    try {
      ...
      await loadPhases(false); // recharge avec la forme complète (élixirs/pratiques inclus)
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Timeline générée : phases CLIQUABLES → onglets (élixirs / pratiques / instructions / check-ins) */}
      {phases.length > 0 && (
        <div className="space-y-3">
          <p className="text-xs font-caps uppercase tracking-wider text-brun-mid">
            Timeline · {phases.length} phases <span className="text-brun-mid/40">— clique une phase pour ses élixirs, pratiques, instructions…</span>
          </p>
          <div className="flex gap-2 overflow-x-auto pb-1">
            {phases.map((p) => (
              <button
                key={p.id}
                onClick={() => setSelectedPhaseId(p.id === selectedPhaseId ? null : p.id)}
                className={`flex-shrink-0 min-w-[130px] border rounded-[8px] p-2 text-left transition-all ${
                  selectedPhaseId === p.id ? "border-or-sacre ring-2 ring-or-sacre/30 bg-or-sacre/5" : "border-or-pale bg-creme-sacree hover:border-or-sacre"
                }`}
              >
                ...
                {p.phaseElixirs && p.phaseElixirs.length > 0 && (
                  <p className="text-[10px] font-ui text-or-sacre mt-0.5">🍯 {p.phaseElixirs.length}</p>
                )}
              </button>
            ))}
          </div>
          ...
          {selectedPhase && (
            <PhaseDetail phase={selectedPhase} allPhases={phases} onUpdate={() => loadPhases(false)} />
          )}
        </div>
      )}
    </div>
