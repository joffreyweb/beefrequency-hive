...
const REAL_MORNING: string[] = [
  "Niveau d'énergie — échelle 1 à 10 (1 = épuisé · 10 = vibrant et aligné)",
  "Qualité du sommeil — échelle 1 à 10 (1 = agité · 10 = régénérant)",
  "Type de sommeil — Léger/flou · Profond/réparateur · Rêves présents · Réveils multiples · Long endormissement · Nuit continue",
  "Ai-je rêvé ? — Oui / Non / Je ne sais pas",
  "Rêves / messages reçus dans la nuit — texte",
  "La gratitude pour cette journée — texte",
];

const REAL_EVENING: string[] = [
  "Trois micro-moments où j'ai été content(e) de moi (3 champs)",
  "Quatre « piments » de gratitude (moment agréable · sensation corporelle · quelque chose reçu · chose faite pour soi)",
  "Une chose que je reconnais chez moi aujourd'hui",
  "Clôture douce — une phrase",
];

function CheckinsTab({ phase, onUpdate }: { phase: ClientPhase; allPhases: ClientPhase[]; onUpdate: () => void }) {
  ...
  function renderSection(title: string, enabled: boolean, setEnabled: (v: boolean) => void, questions: string[]) {
    return (
      <div className="border border-or-pale/50 rounded-lg p-4">
        <label className="flex items-center gap-3 cursor-pointer mb-2">
          ...
          <span className="text-sm font-ui text-brun-chaud font-medium">{title}</span>
        </label>
        <ul className="pl-7 space-y-1 list-disc list-inside">
          {questions.map((q, i) => (
            <li key={i} className="text-xs font-ui text-brun-mid/70">{q}</li>
          ))}
        </ul>
      </div>
    );
  ...
  return (
    <div className="bg-cire-chaude border border-or-pale rounded-[10px] p-5 space-y-5">
      <p className="text-[11px] font-ui text-brun-mid/60 italic">
        Aperçu des questions réellement posées au client (identiques pour tout le parcours). Les interrupteurs activent ou désactivent le check-in ; le contenu des questions est fixe.
      </p>
      {renderSection("Check-in matin", morningEnabled, setMorningEnabled, REAL_MORNING)}
      {renderSection("Check-in soir", eveningEnabled, setEveningEnabled, REAL_EVENING)}
      <div>
