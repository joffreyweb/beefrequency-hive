...
export default function ClientElixirsPage() {
  ...
  // Phase active = celle dont [startDate, endDate] contient aujourd'hui
  ...
  const elixirs = activePhase?.phaseElixirs ?? [];
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-brun-mid font-ui text-sm mt-1">
          {activePhase
            ? `Your elixirs for ${phaseName(activePhase)}`
            : "Your assigned elixirs"}
        </p>
      </div>
      ...
      {elixirs.length === 0 ? (
        <div className="bg-cire-chaude border border-or-pale rounded-sm p-8 text-center">
          <p className="text-sm font-ui text-brun-mid/60">
            No elixirs assigned for your current phase yet
          </p>
        </div>
      ) : (
